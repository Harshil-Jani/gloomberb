...
The loader reads current submissions and at most the latest annual and quarterly primary documents. Each document can succeed independently. For example, the April 29, 2026 annual amendment is unsupported by the table parser, but the later Q2 filing still supplies its directly reported Q4. A later quarterly amendment likewise does not erase a qualified annual filing's historical quarters. The loader never fetches a preceding filing in place of the selected unsupported amendment, and never relabels another filing's evidence with the amendment's date.

A complete two-document table snapshot is reusable for six hours. A partial result preserves qualified dated cohorts and retries after one minute in a separate cache, without extending the previous complete snapshot's expiry. Total acquisition failure retains the latest qualified snapshot and its dates. Companyfacts can succeed independently of table acquisition. The optional table wait is bounded to 750 ms. Native and Cloud responses carry an internal retry deadline so the local financial cache revisits incomplete acquisition after one minute and can receive a completed background fetch. This changes cache eligibility, not any financial observation or its source date.
